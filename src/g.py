...
class GoalPublisher(object):
    ...
    def docking_procedure(self, target_digit):
        rospy.loginfo("Starting parking procedure, target digit: %s", target_digit)
        coords = [
            {'x': 4.0, 'y': -17.5},
            {'x': 4.0, 'y': -13.5},
            {'x': 4.0, 'y': -9.0},
        ...
        for idx, coord in enumerate(coords):
            ...
            # Using 0° orientation here (modify if needed)
            ...
            rospy.loginfo("Navigating to parking spot %d: coordinates (%.2f, %.2f)", idx + 1, coord['x'], coord['y'])
            self.reached = False
            rate = rospy.Rate(10)
            while not rospy.is_shutdown() and not self.reached:
                ...
                if self.last_amcl_time is not None:
                    if (current_time - self.last_amcl_time) >= self.stop_duration_required:
                        rospy.loginfo("Arrived at parking spot %d", idx + 1)
                        self.reached = True
                rate.sleep()

            # For the first three parking spots, capture front camera image and perform OCR recognition
            if idx < 3:
                rospy.loginfo("Capturing front camera image for OCR recognition at parking spot %d...", idx + 1)
                if self.front_img is None:
                    rospy.logwarn("Front camera image data not available, proceeding to the next parking spot")
                    continue
                captured_img = self.front_img.copy()
                # Preprocess image and perform OCR digit recognition
                _, _, processed_img = self.preprocess_image(captured_img)
                recognized_digit = self.ocr_digits(processed_img)
                rospy.loginfo("OCR recognition result at parking spot %d: %s", idx + 1, recognized_digit)
                if recognized_digit == target_digit:
                    rospy.loginfo("Parking spot %d digit matches target, moving forward 1.5 meters", idx + 1)
                    forward_twist = Twist()
                    forward_twist.linear.x = 0.5  # Use a slower speed, unit m/s
                    ...
                    rospy.loginfo("Moved forward 1.5 meters, docking complete")
                    break  # Docking successful, exit loop
                else:
                    rospy.loginfo("Parking spot %d digit does not match target, navigating to next parking spot", idx + 1)
                    continue
            else:
                # The fourth parking spot: directly dock without image capture
                rospy.loginfo("Arrived at the fourth parking spot, no image capture required, task five complete")
    ...
    def run(self):
        ...
        if next_goal is None:
            rospy.logwarn("Can not calculate!!!")
            return

        # Use the computed BC_mid_global as the next target (here only updating the y coordinate),
        # and set yaw to 180° (i.e., the robot faces the opposite direction)
        rospy.loginfo("BC_mid_global: %s, yaw is 180°", next_goal)
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        self.goal_pose.header.stamp = rospy.Time.now()
        self.goal_pub.publish(self.goal_pose)
        rospy.loginfo("New target point published, waiting to reach the target...")
        ...
        while not rospy.is_shutdown() and not self.reached:
            ...
            if self.last_amcl_time is not None:
                if (current_time - self.last_amcl_time) >= self.stop_duration_required:
                    rospy.loginfo("/amcl_pose has stopped publishing for 3 seconds, entering bridge crossing procedure...")
        ...
        rate = rospy.Rate(10)  # Control loop frequency at 10Hz
        forward_twist = Twist()
        forward_twist.linear.x = 15  # Set vehicle forward speed (unit: m/s)
        forward_twist.angular.z = 0.0  # Maintain straight movement
        self.cmd_vel_pub.publish(forward_twist)
        ...
        # After moving forward, publish stop command
        stop_twist = Twist()  # Default all parameters to 0, i.e., stop moving
        self.cmd_vel_pub.publish(stop_twist)
        rospy.loginfo("Vehicle has moved forward 5 meters and stopped.")
