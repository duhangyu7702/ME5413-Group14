...
class GoalPublisher(object):
    ...
    def run(self):
        ...
        yolo_path = os.path.join(self.image_folder, "y.jpg")
        cv2.imwrite(yolo_path, detection_frame)
        cv2.waitKey(10000)
        cv2.destroyAllWindows()

        rospy.loginfo("发布最终目标点，并设定正确的旋转角度...")
        self.goal_pose.pose.position.x = 9.8
        self.goal_pose.pose.position.y = -21.5
        self.goal_pose.pose.orientation.z = 0.30
        self.goal_pose.pose.orientation.w = 0.95
        self.reached = False
        self.goal_pose.header.stamp = rospy.Time.now()
        ...
        while not rospy.is_shutdown() and not self.reached:
            ...
            if self.last_amcl_time is not None:
                if (current_time - self.last_amcl_time) >= self.stop_duration_required:
                    rospy.loginfo("/amcl_pose 已停止发布 3 秒，进入激光雷达旋转流程...")
                    self.reached = True
            rate.sleep()
